<?php
	...
	if ((!isset($_GET['iframe'])&&@$_GET['iframe']!=true)||($fct!='view'&&$op=='videos')) {
		...
		if ($vid>0) {
			...
			$video = $videos_handler->get($videos_handler->insert($video, true));
			$category = $category_handler->get($video->getVar('cid'));
			if (is_object($category)) {
				$category->setVar('hits', $category->getVar('hits')+1);
				$category = $category_handler->get($category_handler->insert($category, true));
				$cathits=true;
			}
			...
			switch ($fct) {
				case 'add':
					if (!is_array($_SESSION['vod']['cart'])||!isset($_SESSION['vod']['cart'])) {
						@$sessions_handler->createCart();
						setcookie('cart', $_SESSION['vod']['cart'], 3600*24*30);
					}
					$sessions = $sessions_handler->get($_SESSION['vod']['cart']['sessid']);
					$sessions->addVideo($video);
					if ($GLOBALS['vodModuleConfig']['cart']==false) {
						$uri = $_SERVER["PHP_SELF"].'?op=cart&fct=checkout';
						redirect_header($uri, 10, _MN_VOD_MSG_CHECKOUTITEMANDPAY);
						exit;
					}
				...
				case 'checkout':
					...
					if (strlen($sessions->getVar('email'))==0) {
						if (!eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z0-9-]+)$", $_POST['email'])) {
							redirect_header($uri, 10, _MN_VOD_MSG_EMAILADDRESS_NOTVALID);
							exit;
						}
						$sessions->setVar('email', $_POST['email']);
						$save=true;
					}
					if (strlen($sessions->getVar('name'))==0) {
						if (empty($_POST['name'])) {
							redirect_header($uri, 10, _MN_VOD_MSG_NAME_EMPTY);
							exit;
						}
						$sessions->setVar('name', $_POST['name']);
						$save=true;
					}
					if (strlen($sessions->getVar('pass'))<>32) {
						if (empty($_POST['pass'])) {
							redirect_header($uri, 10, _MN_VOD_MSG_PASSWORD_EMPTY);
							exit;
						}
						$sessions->setVar('pass', md5($_POST['pass']));
						$save=true;
					}
					if ($save==true)
						$sessions = $sessions_handler->get($sessions_handler->insert($sessions, true));
					$GLOBALS['xoopsLogger']->activated = false;					
					$GLOBALS['xoopsTpl']->assign('xoConfig', $GLOBALS['vodModuleConfig']);
					$GLOBALS['xoopsTpl']->assign('cart', $sessions->getCartArray(true));
					$GLOBALS['xoopsTpl']->assign('session', $sessions->toArray(true));
					if ($sessions->getVar('total')>0) {
						$GLOBALS['xoopsTpl']->display('db:'.$xoopsOption['template_main']);
					} else {
						$sessions->setVar('mode', '_VOD_ENUM_PAID');
						$sessions_handler->insert($sessions, true);
						redirect_header($uri, 10, _MN_VOD_MSG_CART_MARKEDPAID);
						exit;
		...
		if ($vid!=0) {
			...
			$videos->setVar('views', $videos->getVar('views')+1);
			$videos = $videos_handler->get($videos_handler->insert($videos, true));
		}
		
		if (is_object($videos)) {
			if ($videos->getVar('price')>0) {
				...
				if (isset($_REQUEST['sessid'])) {
					$sessions = $sessions_handler->get($_REQUEST['sessid']);
					$_SESSION['vod'][$vid]['sessid'] = $_REQUEST['sessid'];
				} elseif (isset($_SESSION['vod'][$vid]['sessid'])) {
					$sessions = $sessions_handler->get($_SESSION['vod'][$vid]['sessid']);
				}
				...
				if ($_REQUEST['token']!=sha1($sessions->getVar('paidkey').$videos->getVar('token'))&&!isset($_SESSION['vod'][$vid]['main'])) {
					redirect(XOOPS_URL, 10, _NOPERM);
					exit;
				} else {
