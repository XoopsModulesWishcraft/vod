...
class VodVideos extends XoopsObject
{
    ...
    function getHTML($block = false, $width=0, $height=0, $agent = '', $ip = '', $preview = false) {
    	if (empty($ip))
    		$ip = $this->getIP();
    	if (empty($agent))
    	...
    	$videos['id'] = $this->getReference($block);
    ...
    function getJS($block=false, $width = '', $height = '', $preview = false) {
		...
		if ($block==false) {
			$_SESSION['vod'][$this->getIP()]['div_'.$this->getReference($block)]['passkey'] = sha1(XOOPS_LICENSE_KEY.$this->_ModConfig['salt'].$this->getIP().date('Ymdhis'));
			$_SESSION['vod'][$this->getIP()]['div_'.$this->getReference($block)][$_SESSION['vod'][$this->getIP()]['div_'.$this->getReference($block)]['passkey']]['vid'] = $this->getVar('vid');
			$_SESSION['vod'][$this->getIP()]['div_'.$this->getReference($block)][$_SESSION['vod'][$this->getIP()]['div_'.$this->getReference($block)]['passkey']][$this->getVar('vid')]['block'] = ($block==true?'1':'0');
			$_SESSION['vod'][$this->getIP()]['div_'.$this->getReference($block)][$_SESSION['vod'][$this->getIP()]['div_'.$this->getReference($block)]['passkey']][$this->getVar('vid')]['width'] = $width;
			$_SESSION['vod'][$this->getIP()]['div_'.$this->getReference($block)][$_SESSION['vod'][$this->getIP()]['div_'.$this->getReference($block)]['passkey']][$this->getVar('vid')]['height'] = $height;
			$_SESSION['vod'][$this->getIP()]['div_'.$this->getReference($block)][$_SESSION['vod'][$this->getIP()]['div_'.$this->getReference($block)]['passkey']][$this->getVar('vid')]['resolve'] = 'div_'.$this->getReference($block);
		}
		return 'vod_dojson_videos("'.XOOPS_URL.'","'._MI_VOD_DIRNAME.'","'.($block==false?$_SESSION['vod'][$this->getIP()]['div_'.$this->getReference($block)][$_SESSION['vod'][$this->getIP()]['div_'.$this->getReference($block)]['passkey']][$this->getVar('vid')]['resolve']:'%resolve%').'", "'.$preview.'");';
    }

    function getInsecureJS($block=false, $width = '', $height = '', $preview=false) {
    	static $_loadedJS = false;
    	xoops_loadLanguage('modinfo', 'vod');
		$mode = $this->getModeWithUserAgent($_SERVER['HTTP_USER_AGENT']);
		if (is_object($GLOBALS['xoTheme'])&&$_loadedJS==false) {
		...
		switch ($action) {
			case 'vod_file':
			return "vod('".$this->getReference($block)."', {src: '".$out[0]."', wmode: 'opaque', width: '".$out[20]."', height: '".$out[21]."'}, {
	                clip: {
	                    url: '".$out[3]."',
	            	    autoPlay: ".$out[1].",
	            	    onBegin: function () {
		...
		case 'videojs_file':
		case 'videojs_stream':
			return "var myVideos = VideoJS.setup('".$this->getReference($block)."', {
