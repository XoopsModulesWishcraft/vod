...
class VodSessionsHandler extends XoopsPersistableObjectHandler
{
    ...
    function getSessIDFromUID($uid, $as_array = true) {
    	$objects = $this->getObjects(new Criteria('uid', $uid), true);
    	$ret = array();
    	foreach($objects as $sessid => $object) {
    		if ($as_array==true) {
    			$ret[$sessid] = $sessid;
    		} else {
    			$ret[$sessid] = $object;
