...
function b_vod_block_videos_show( $options )
{
	...
	if (!isset($GLOBALS['_done'][$block['id']])) {
		...
		if ($GLOBALS['vodModuleConfig'][$mode.'_secure']==true) {
			...
			$_SESSION['vod'][$videos->getIP()][$block['id']][$_SESSION['vod'][$videos->getIP()][$block['id']]['passkey']][$videos->getVar('vid')]['preview'] = $preview;
			$_SESSION['vod'][$videos->getIP()][$block['id']][$_SESSION['vod'][$videos->getIP()][$block['id']]['passkey']][$videos->getVar('vid')]['state'] = 'block';
			
			$GLOBALS['xoTheme']->addScript('', array('type'=>'text/javascript'), str_replace('%resolve%', $block['id'], $videos->getJS(true, $block['width'], $block['height'], $preview)));
			$block['html'] = false;
		} else {
			$GLOBALS['xoTheme']->addScript('', array('type'=>'text/javascript'), $videos->getInsecureJS(true, $block['width'], $block['height'], $preview));
			$block['html'] = $videos->getHTML(true, $block['width'], $block['height'], $_SERVER['HTTP_USER_AGENT'], $videos->getIP(), $preview);
		}
