...
<!-- End Form Validation JavaScript //-->');
	...
	case "log":	
		...
		foreach ($log_handler->filterFields() as $id => $key) {
			$GLOBALS['xoopsTpl']->assign(strtolower(str_replace('-','_',$key).'_th'), '<a href="'.$_SERVER['PHP_SELF'].'?start='.$GLOBALS['start'].'&limit='.$GLOBALS['limit'].'&sort='.$key.'&order='.(($key==$GLOBALS['sort'])?($GLOBALS['order']=='DESC'?'ASC':'DESC'):$GLOBALS['order']).'&op='.$GLOBALS['op'].'&filter='.$GLOBALS['filter'].'">'.(defined('_VS_AM_TH_'.strtoupper(str_replace('-','_',$key)))?constant('_VS_AM_TH_'.strtoupper(str_replace('-','_',$key))):'_VS_AM_TH_'.strtoupper(str_replace('-','_',$key))).'</a>');
	...
	case "videos":	
		...
		switch ($fct)
		{
			...
			case "list":				
				...
				$videoss = $videos_handler->getObjects($criteria, true);
				foreach($videoss as $cid => $videos) {
					if (is_object($videos))
						$GLOBALS['xoopsTpl']->append('videoss', $videos->toArray());
				}
