 ...
 * @access 		public
 */
class VodFormSelectCurrency extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function VodFormSelectCurrency($caption, $name, $value = null, $size = 1, $multiple = false, $none = false, $just_code = false, $as_id = false)
    {
        ...
        if ($none==true)
        	$this->addOption('', _MI_VOD_NONE);

        $currency_handler = xoops_getmodulehandler('currency', 'vod');
		$criteria = new criteria('1','1', '=');
    	$criteria->setSort('`name`');
		$criteria->setOrder('ASC');
        foreach($currency_handler->getObjects($criteria, true) as $currency_id => $currency)
			$this->addOption(($as_id==true?$currency_id:$currency->getVar('code')), ($just_code==true?$currency->getVar('code'):$currency->getVar('name'). ' ('.$currency->getVar('code').')'));
    }
