...
class VodCart extends XoopsObject 
{
    ...
    function __construct()
    {
        ...
        $this->initVar('mode', XOBJ_DTYPE_ENUM, '_VOD_ENUM_UNPAID', false, false, false, array('_VOD_ENUM_UNPAID','_VOD_ENUM_PAID','_VOD_ENUM_CANCELED'));
	...
	function toArray($xpayment=false) {
    	...
    	if (!isset($_vids[$this->getVar('vid')])&&$this->getVar('vid')!=0) {
    		$_vids[$this->getVar('vid')] = $videos_handler->get($this->getVar('vid')); 
    	}
    	if ($this->getVar('vid')!=0) {
    		if (is_object($_vids[$this->getVar('vid')]))
    			$ret['vid'] = $_vids[$this->getVar('vid')]->getVar('name');
    	}
    	
		foreach(array('added', 'paid', 'notified', 'claimed', 'closed', 'expires', 'created', 'updated') as $key) {
...
class VodCartHandler extends XoopsPersistableObjectHandler
{
	...
	function filterFields() {
		return array('crtid', 'mode', 'sessid', 'cid', 'vid', 'pid', 'tokens', 'cost_usd', 'cost_aud', 'cost', 'currency', 'total', 'added', 'notified', 'paid', 'claimed', 'expires', 'closed', 'created', 'updated');
	}
	
	
    function VodCartHandler($db) {
