...
class VodLog extends XoopsObject
{
	...
	function runPluginInsert() {
		...
		$func = constant(str_replace('_ENUM', '_ENUM_PLUGINS', $this->getVar('type'))).constant(str_replace('_ENUM', '_ENUM_PLUGINS', $this->getVar('mode'))).ucfirst($class).$cause;
    ...
    function createLog($_file, $_line, $_object, $_comment = '', $_id = 0, $_uid = 0) 
    {
		...
		$name = null;
	   	if (is_object($_object)&&isset($_object->vars)) {
	   		$name = @$_object->getName(false, true, true, false, false);
